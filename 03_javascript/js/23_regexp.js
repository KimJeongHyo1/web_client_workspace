 ...
 * - 조건/반복처리를 내부적으로 진행해서 간결하게 작성 가능
 * - 문법자체 숙지가 어려운편
 * - 언어독립적으로 작동함
 */
document.querySelector("#btn1").addEventListener('click', () => {
    ...
    // 정규식 메소드
    // RegExp#test(string): boolean  --> 가장 많이 사용함
    console.log(re.test("abc123")); // 숫자있어서 true
    console.log(re.test("xyzㅋㅋㅋ")); // 숫자없으니까 false

    // 문자열 메소드
    // String#split(re(=구분자 정규식)): string[]
    ...
    // String#search(re): number 매칭된 문자열의 인덱스를 반환
    console.log("a1b2c3d".search(re)); // 1  1번지에서 발견됐어
    console.log("xyzㅋㅋㅋ".search(re)); // -1  값이없어
});
...
document.querySelector("#btn3").addEventListener('click', (e) => {
    ...
    area.innerHTML = `<p>${src.replace(/j/gi, '<mark>$&</mark>')}</p>`; // j 전부 mark
    area.innerHTML += `<p>${src.replace(/^j/gi, '<mark>$&</mark>')}</p>`; // 처음 j만 mark
    area.innerHTML += `<p>${src.replace(/x/gi, '<mark>$&</mark>')}</p>`; // x 전부 mark
    area.innerHTML += `<p>${src.replace(/x$/gi, '<mark>$&</mark>')}</p>`; // 마지막 x만 mark
...
document.querySelector("#btn4").addEventListener('click', (e) => {
    const re = /^a.b/; // a나오고 임의의 문자나오고 b나오는지
...
document.querySelector("#btn5").addEventListener('click', (e) => {
    ...
    // area.innerHTML += `<p>${src.replace(/^[0-9A-z]/gi, '<mark>$&</mark>')}</p>`; // 이렇게쓰지말것
    ...
    console.log(re2.test('abc12')); // false
...
document.querySelector("#btn8").addEventListener('click', (e) => {
    ...
    area.innerHTML += `<p>${src.replace(/\./g, '<mark>$&</mark>')}</p>`;
    area.innerHTML += `<p>${src.replace(/[$]/g, '<mark>$&</mark>')}</p>`; // []안에서는 escaping불필요 \는 제외
    area.innerHTML += `<p>${src.replace(/[.]/g, '<mark>$&</mark>')}</p>`; 
