...
const test1 = () => {
    ...
    // 기본값지정
    // const [l, m, n] = [10, 20]; // 10 20 undefined
    const [l, m, n = 100] = [10, 20]; // 10 20 100
    console.log(l, m, n);

    // 나머지파라미터(변수)
    const [i, j, ...k] = [ 'a', 'b', 'c', 'd', 'e'];
    console.log(i, j, k); // a b (3) ['c', 'd', 'e']
};
...
printSns(user);
 ...
 // 홈페이지 : ...
 // 전화번호 : ...(대표), ...
 // 지점1 : 강남 지원 (서울시 강남구 역삼동)
 // 지점2 : 종로 지원 (서울시 강남구 역삼동)
 // 지점3 : 당산 지원 (서울시 강남구 역삼동)
};
